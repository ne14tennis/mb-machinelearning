...
#Comparing Models through confusion matrix heatmap
    ...
    plt.subplot(2,3,1)
    sns.heatmap(confusion_matrix(y_test,y_pred_log)/np.sum(confusion_matrix(y_test, y_pred_log)), annot=True, fmt='.2%',cmap='Blues')
    plt.title('Logistic')

    # Decision Tree confusion matrix heatmap
    plt.subplot(2,3,2)
    sns.heatmap(confusion_matrix(y_test,y_pred_dt)/np.sum(confusion_matrix(y_test,y_pred_dt)), annot=True, fmt='.2%',cmap='Blues')
    plt.title('Decision Tree')

    # Random Forest
    plt.subplot(2,3,3)
    sns.heatmap(confusion_matrix(y_test, y_pred_rf)/np.sum(confusion_matrix(y_test, y_pred_rf)), annot=True, fmt='.2%',cmap='Blues')
    plt.title('Random Forest')


    # XGBoost
    plt.subplot(2,3,4)
    sns.heatmap(confusion_matrix(y_test,y_pred_xg)/np.sum(confusion_matrix(y_test,y_pred_xg)), annot=True, fmt='.2%',cmap='Blues')
    plt.title('XGBoost')

    #MLP
    plt.subplot(2,3,5)
    sns.heatmap(confusion_matrix(y_test, y_pred_MLP)/np.sum(confusion_matrix(y_test, y_pred_MLP)), annot=True, fmt='.2%',cmap='Blues')
    plt.title('MLP')
